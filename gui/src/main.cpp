...
typedef struct {
  ...
  double
      *filterHistory; // History buffer for FIR filter (holds previous samples)
  fftw_plan p;
  int inputChannels;
  int startIndex; // First index of our FFT output to display in the spectrogram
  int spectroSize; // Number of elements in our FFT output to display from the
                   // start index
} streamCallbackData;
...
static int getThr5AlsaCardIndex() {
  ...
  while ((entry = readdir(asoundDir)) != NULL) {
    ...
    char usbidPath[64];
    snprintf(usbidPath, sizeof(usbidPath), "/proc/asound/card%d/usbid",
             parsedIndex);
...
void audioThreadFunc() {
  ...
  spectroData->filterHistory =
      (double *)calloc(FILTER_ORDER, sizeof(double)); // Zero-initialized
  ...
  spectroData->spectroSize =
      min(std::ceil(sampleRatio * SPECTRO_FREQ_END), FRAMES_PER_BUFFER / 2.0) -
  ...
  if (useAlsaDeviceString) {
    ...
    snprintf(deviceCandidates[0], sizeof(deviceCandidates[0]), "plughw:%d,0",
             thr5CardIndex);
    snprintf(deviceCandidates[1], sizeof(deviceCandidates[1]), "hw:%d,0",
             thr5CardIndex);
    snprintf(deviceCandidates[2], sizeof(deviceCandidates[2]), "dsnoop:%d,0",
             thr5CardIndex);
    snprintf(deviceCandidates[3], sizeof(deviceCandidates[3]),
             "sysdefault:%d,0", thr5CardIndex);
    snprintf(deviceCandidates[4], sizeof(deviceCandidates[4]), "sysdefault:%d",
             thr5CardIndex);
    ...
    for (int d = 0; d < 5 && !opened; d++) {
      for (int c = 0; c < 2 && !opened; c++) {
        ...
        openErr = Pa_OpenStream(&stream, &inputParameters, NULL, SAMPLE_RATE,
                                FRAMES_PER_BUFFER, paNoFlag, streamCallback,
                                spectroData);
        if (openErr == paNoError) {
          snprintf(alsaDeviceString, sizeof(alsaDeviceString), "%s",
                   deviceCandidates[d]);
    ...
    if (!opened) {
      ...
      if (pulseDevice != paNoDevice) {
        ...
        if (pulseInfo != NULL && pulseInfo->maxInputChannels > 0) {
          int pulseChannels =
              pulseInfo->maxInputChannels >= NUM_CHANNELS ? NUM_CHANNELS : 1;
    ...
    inputParameters.suggestedLatency =
        selectedDeviceInfo->defaultLowInputLatency;

    openErr =
        Pa_OpenStream(&stream, &inputParameters, NULL, SAMPLE_RATE,
                      FRAMES_PER_BUFFER, paNoFlag, streamCallback, spectroData);
    if (openErr != paNoError) {
