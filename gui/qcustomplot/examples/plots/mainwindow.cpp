...
void MainWindow::setupAdvancedAxesDemo(QCustomPlot *customPlot) {
  ...
  for (int i = 0; i < dataRandom.size(); ++i) {
    ...
    dataRandom[i].value =
        std::rand() / (double)RAND_MAX - 0.5 + dataRandom[qMax(0, i - 1)].value;
  }
  x3 << 1 << 2 << 3 << 4;
  y3 << 2 << 2.5 << 4 << 1.5;
...
void MainWindow::setupFinancialDemo(QCustomPlot *customPlot) {
  ...
  QCPBars *volumeNeg = new QCPBars(volumeAxisRect->axis(QCPAxis::atBottom),
                                   volumeAxisRect->axis(QCPAxis::atLeft));
  for (int i = 0; i < n / 5; ++i) {
    int v = std::rand() % 20000 + std::rand() % 20000 + std::rand() % 20000 -
            10000 * 3;
...
void MainWindow::setupPolarPlotDemo(QCustomPlot *customPlot) {
  ...
  /* This is how we could set the angular axis to show pi symbols instead of
  degree numbers: QSharedPointer<QCPAxisTickerPi> ticker(new QCPAxisTickerPi);
  ...
  */
  ...
  angularAxis->grid()->setAngularPen(
      QPen(QColor(200, 200, 200), 0, Qt::SolidLine));
...
void MainWindow::realtimeDataSlot() {
  ...
  if (key - lastPointKey > 0.002) // at most add point every 2 ms
  {
    ...
    ui->customPlot->graph(1)->addData(
        key,
        qCos(key) + std::rand() / (double)RAND_MAX * 0.5 * qSin(key / 0.4364));
...
#else
  QPixmap pm = qApp->primaryScreen()->grabWindow(
      0, this->x() - 7, this->y() - 7, this->frameGeometry().width() + 14,
